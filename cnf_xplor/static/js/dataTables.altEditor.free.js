        ...
        altEditor.prototype,
        {
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                // data += "</form>";
                // Adrian Adds
                // This splits it into the left (label) and the right (input)
                let name = "REASON";
                let t = "Reason for change";

                data += "<div style='margin-left: initial;margin-right: initial;' class='form-group row' id='alteditor-row-" + this._quoteattr(name) +"'>";
                data += "<div class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:4px;'>";
                data += "<label for='" + name + "'>" + t + ":</label></div>";
                data += "<div class='col-sm-8 col-md-8 col-lg-8'>";

                // Add the input box
                let j_last = columnDefs.length - 1
                data += "<input type='" + this._quoteattr("text")
                + "' id='" + this._quoteattr(name)
                + "' pattern='" + this._quoteattr(columnDefs[j_last].pattern)
                + "' title='" + this._quoteattr(columnDefs[j_last].hoverMsg)
                + "' name='" + this._quoteattr(t)
                + "' placeholder='" + this._quoteattr("")
                + "' data-special='" + this._quoteattr(columnDefs[j_last].special)
                + "' data-errorMsg='" + this._quoteattr(columnDefs[j_last].msg)
                + "' data-uniqueMsg='" + this._quoteattr(columnDefs[j_last].uniqueMsg)
                + "' data-unique='" + columnDefs[j_last].unique
                + "' "
                + (columnDefs[j_last].readonly ? ' readonly ' : '')
                + (columnDefs[j_last].disabled ? ' disabled ' : '')
                + (columnDefs[j_last].required ? ' required ' : '')
                + (columnDefs[j_last].maxLength == false ? "" : " maxlength='" + columnDefs[j_last].maxLength + "'")
                + " style='overflow:hidden'  class='form-control  form-control-sm' value=''>";
